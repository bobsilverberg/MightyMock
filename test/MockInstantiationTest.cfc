...
<cfscript>
 ...
 function simpleMockTestWithNoReturns() {
  ...
  function fastMocksShouldNotalterMetaDataAndReturnUniqueObjects() {
     ...
     md2=getMetaData(m2);
     //debug( md2 );
     assertEquals( 'mightymock.MightyMock' , md1.name );
     assertEquals( 'mightymock.MightyMock' , md2.name );
 ...
 function type_safeMockShouldBeKnownType() {
   mock = $('mightymock.test.fixture.Dummy',true);
   assertIsTypeOf(mock,'mightymock.test.fixture.Dummy');
 }

 function typesafeMockShouldBeAcceptedByTypedArg() {
   strict = createObject('component', 'mightymock.test.fixture.AcceptStrictType');
   mock = $('mightymock.test.fixture.Dummy',true);
